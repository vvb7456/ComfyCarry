...
interface UseAnimaModalManagerOptions {
...
/**
 * Anima Tab 的 Modal/Picker 管理 composable。
 * 与 useSdxlModalManager 的区别:
 * - 不使用 checkpoint，改用 UNet / CLIP / VAE 三件套独立 picker
 * - 其余 LoRA / Tagger / LLM / Prompt Editor / Preview Lightbox 完全一致
 */
export function useAnimaModalManager({
  options,
  previewImages,
  prepareTagger,
}: UseAnimaModalManagerOptions) {
  ...
  // ── UNet Picker (CLIP/VAE 改为高级折叠中的 BaseSelect，不再使用 modal) ──

  const showUnetPicker = ref(false)
  const unetSelected = computed(() => new Set(state.value.unet ? [state.value.unet] : []))

  function openUnetPicker() {
    void options.refresh()
    showUnetPicker.value = true
  }
  ...
  return {
    ...
    promptTools,
    // UNet 仅
showUnetPicker, unetSelected, openUnetPicker, onUnetSelect,
    // LoRA
    showLoraPicker,
    loraModalPending,
    loraCountLabel,
