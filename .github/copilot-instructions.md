...
- **本地 ComfyUI 副本**: `~/ComfyUI` 有一个本地 ComfyUI 安装 (非本项目部署)，可用于验证 API 接口、测试插件功能等。启动脚本在 `~/ComfyUI/start_comfy.sh`，使用 venv 环境，tmux 管理
- **SSH 远程命令**: vast.ai SSH 连接较慢，需要输出一段提示后才执行命令。**不要设置过短的超时时间**（推荐 `timeout=0`），不要因为等待时间长就误判为连接中断或执行失败。如果真的出问题用户会手动 Ctrl+C
- **终端命令通用规则**: 所有终端命令统一使用 `timeout=0`，除非命令明确是瞬时完成的（如 echo/cat）。永远不要猜测命令的执行时间

### ask_questions 使用时机 (强制规则)

以下场景**必须立即调用 `ask_questions`**，不得继续自行迭代：

1. **用户打断 (Ctrl+C / 跳过 / 拒绝命令)**: 立刻停止当前操作，调用 `ask_questions` 询问用户意图。**绝对不要**在被打断后继续尝试其他方案
2. **完成任务后**: 不要直接结束对话，使用 `ask_questions` 向用户寻求反馈
3. **遇到无法自行解决的问题**: 不要盲目迭代，使用 `ask_questions` 请求用户协助 (例如: 帮忙在浏览器查看 UI/控制台、在 vast 上重建实例等)
4. **需要用户做出选择时**: 涉及实现方案、行为变更等需要用户决策的场景
5. **验证需要用户配合时**: 例如需要用户在浏览器端查看效果、手动触发某个操作等
